...
Although the full NetBeans data set contains 57 database tables, in this paper only two are used: {\tt bugs} and {\tt changes} (originally, {\tt bugs\_activity}). One table and a few columns were renamed for clarity purposes.
