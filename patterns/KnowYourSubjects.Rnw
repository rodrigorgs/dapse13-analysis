...
@
  ...
  Next, choose a threshold. For that, plot for each possible threshold between 1 and 50, the relative number of verifications vs. the relative size of quality team. In the following source code we use percentages for simplicity purposes. The result is the plot in Figure~\ref{fig:threshold}.

<<figthreshold,include=FALSE>>=
...
<<figthreshold>>
@
  \caption{Plot used to choose a threshold.}
  \label{fig:threshold}
\end{figure}
